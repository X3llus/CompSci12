...
def check(inp): #check inputs to make sure the date is correct
    if len(inp) == 8:
        ...
        if (m == 1 or m == 3 or m == 5 or m == 7 or m == 8 or m == 10 or m == 12) and d <= 31 and d > 0: #checks for months with 31 days
            # if d <= 31 and d > 0:
            return True
        elif (m == 4 or m == 6 or m == 9 or m == 11) and d <= 30 and d > 0: #checks for months with 30 days
            # if d <= 30 and d > 0:
            return True
        elif m == 2:
...
def main(): #main function
    ...
    print ("{} {} {}".format(int(date[0]), month[date[1]], int(date[2])))
